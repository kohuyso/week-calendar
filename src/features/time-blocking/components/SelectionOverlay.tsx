...
export function SelectionOverlay({
  ...
  const minM = Math.max(0, rawMin)
  const maxM = Math.min(1440, rawMin + duration)

  const topPx = (minM / 60) * HOUR_HEIGHT
  const heightPx = ((maxM - minM) / 60) * HOUR_HEIGHT
  ...
  return (
    <div
      ...
      className="absolute left-1 right-1 bg-blue-600/90 text-white rounded-md shadow-md border border-blue-400/80 pointer-events-none z-20 px-2 py-1 flex flex-col justify-start overflow-hidden select-none will-change-[top,height]"
    >
      ...
      <div className="text-[10px] text-blue-100 font-medium tracking-tight truncate mt-1">
